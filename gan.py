...
[os.makedirs(join(abspath(dirname(__file__)), f'{d}/{data_dir}'), exist_ok=True) for d in dir_list]

images_path = join(abspath(dirname(__file__)), f'images/{data_dir}')
...
class Model_GAN(object):
    ...
    def save(self, num):
        ...
        with open(join(abspath(dirname(__file__)), f"Models/{data_dir}/gen.json"), "w+") as json_file:
            json_file.write(gen_json)

        with open(join(abspath(dirname(__file__)), f"Models/{data_dir}/dis.json"), "w+") as json_file:
            json_file.write(dis_json)

        self.GAN.G.save_weights(join(abspath(dirname(__file__)), f"Models/{data_dir}/gen"+str(num)+".h5"))
        self.GAN.D.save_weights(join(abspath(dirname(__file__)), f"Models/{data_dir}/dis"+str(num)+".h5"))
    ...
    def load(self, num):
        ...
        #Generator
        gen_file = open(join(abspath(dirname(__file__)), f"Models/{data_dir}/gen.json"), 'r')
        gen_json = gen_file.read()
        gen_file.close()
        
        self.GAN.G = model_from_json(gen_json)
        self.GAN.G.load_weights(join(abspath(dirname(__file__)), f"Models/{data_dir}/gen"+str(num)+".h5"))

        #Discriminator
        dis_file = open(join(abspath(dirname(__file__)), f"Models/{data_dir}/dis.json"), 'r')
        dis_json = dis_file.read()
        dis_file.close()
        
        self.GAN.D = model_from_json(dis_json)
        self.GAN.D.load_weights(join(abspath(dirname(__file__)), f"Models/{data_dir}/dis"+str(num)+".h5"))
    ...
    def eval2(self, num = 0):
        ...
        im2 = self.generator.predict(noise(12))
        ...
        x.save(join(abspath(dirname(__file__)), f"Results/{data_dir}/i")+str(num)+".png")
        ...
        for i in range(12):
            ...
            im_gen.save(join(abspath(dirname(__file__)),f"Generated_images/{data_dir}/gen_"+str(num)+ str(i) + ".png"))
...
while(model.GAN.steps < 20000):
    
    #print("\n\n\n\nRound " + str(model.GAN.steps) + ":")
    model.train()
    if model.GAN.steps % 50 == 0:
        print("\nRound: " + str(model.GAN.steps))
    
    if model.GAN.steps % 100 == 0:
        print("\n\n\n\nRound " + str(model.GAN.steps) + ":")
